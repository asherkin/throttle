...
class Crash
{
    ...
    public function list_crashes(Application $app)
    {
        ...
        $check_user = $user['admin'] ? '' : 'AND owner = ?';
        $crashes = $app['db']->executeQuery('SELECT crash.id, UNIX_TIMESTAMP(crash.timestamp) as timestamp, crash.owner, crash.cmdline, frame.module, frame.function, frame.file, frame.line, frame.offset FROM crash JOIN frame ON crash.id = frame.crash AND crash.thread = frame.thread AND frame.frame = 0 WHERE crash.processed = 1 ' . $check_user . ' ORDER BY crash.timestamp DESC LIMIT 100', array($user['id']))->fetchAll();
