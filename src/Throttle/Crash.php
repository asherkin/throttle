...
class Crash
{
    ...
    {
        $crash = $app['db']->executeQuery('SELECT id, UNIX_TIMESTAMP(crash.timestamp) as timestamp, owner, metadata, cmdline, thread, processed, failed, stackhash FROM crash WHERE id = ?', array($id))->fetch();
